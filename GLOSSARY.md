...
- **2026-08-08 (spot-check review across ch1-8)** — একটা সম্পূর্ণ বই-জোড়া cross-check-এর সময় (upstream diff + content spot-check) ধরা পড়েছিল যে chapter 8-এর `policy.asc`-এ আগে "enforce করা" → "প্রয়োগ করা" ফিক্স করা হলেও, একই chapter-এর `hooks.asc`-এ একই phrase ("policy enforce করা"/"policy enforce করতে") দুই জায়গায় থেকে গিয়েছিল, English-এই। এখন `hooks.asc`-ও `policy.asc`-এর সাথে সামঞ্জস্যপূর্ণ করা হলো। (`config.asc` আর `import-custom.asc`-এ "enforce" শব্দটা শুধু `_an_example_git_enforced_policy` anchor ID-এর ভেতরে আছে বলে সেগুলো ঠিকই আছে, পরিবর্তনের দরকার নেই — anchor কখনো translate হয় না।)
- **2026-08-08 (appendix A/B prep)** — Appendix A (Git in Other Environments) আর B (Embedding Git) অনুবাদ শুরুর আগে chapter 1-10-এ real precedent চেক করা হলো: Command Line → "কমান্ড লাইন" (transliterate, chapter 2 থেকেই প্রতিষ্ঠিত), Terminal → "টার্মিনাল" (transliterate, chapter 3/8-এ প্রতিষ্ঠিত), GUI/IDE (English, universal acronym, SSH/API-এর মতোই ক্যাটেগরি), Shell (English, প্রতিষ্ঠিত), Library → "লাইব্রেরি" (transliterate, chapter 9/10-এ প্রতিষ্ঠিত)। নতুন প্রোডাক্ট/টুলের proper noun (Visual Studio, Visual Studio Code, JetBrains, Sublime Text, Bash, Zsh, PowerShell, libgit2, JGit, go-git, Dulwich) — সব English-ই থাকবে, GitHub/GitLab-এর মতোই প্রোডাক্ট-নামের ক্যাটেগরি। প্রোগ্রামিং-ভাষা-নির্দিষ্ট টার্ম (binding, wrapper, thread-safe, callback, garbage collected) — Hook/Daemon-এর প্যাটার্নে English।
- **2026-08-08 (chapter 10 prep)** — Chapter 10 (Git Internals) অনুবাদ শুরুর আগে টার্ম prep করা হলো। **English-ই থাকবে** (Git-এর নিজস্ব internals পরিভাষা, Daemon/Hook/Ref-Refspec-এর মতোই ক্যাটেগরি, প্রায়ই literal কমান্ড/ফাইলের নামের সাথে ওভারল্যাপ করে): Plumbing/Porcelain, object/commit object/tree object/tag object (blob/tree আগে থেকেই English প্রতিষ্ঠিত), loose object, packfile, delta, pack index, zlib, SHA-1 (আগে থেকেই প্রতিষ্ঠিত), symref, packed-refs, upload-pack/receive-pack, smart/dumb protocol, sideband, pkt-line, `git gc`/`git prune`/`git fsck`/`git count-objects`/`git verify-pack` (literal কমান্ড-নাম), environment variable (আগে থেকেই প্রতিষ্ঠিত, chapter 2/4)। **Native বাংলায়**: content-addressable filesystem-এর generic ব্যাখ্যামূলক গদ্য (rule 24 অনুযায়ী)।
- **2026-08-08 (chapter 9 prep)** — Chapter 9 (Git and Other Systems) অনুবাদ শুরুর আগে টার্ম prep করা হলো। **English-ই থাকবে** (proper noun / other-VCS-specific feature নাম, Submodule/Bisect-এর মতোই ক্যাটেগরি, প্রায়ই literal কমান্ডের সাথে ওভারল্যাপ করে): Subversion/SVN, `git svn`/dcommit, revprop, trunk/branches/tags (SVN-এর literal directory নাম, rule 13), Mercurial/hg, changeset (chapter 3-এর nutshell.asc-এ আগে থেকেই English প্রতিষ্ঠিত), bookmark (hg-টার্ম), Perforce/p4, depot, changelist, client spec, `fast-import`/`fast-export`, marks file। **Native বাংলায় অনুবাদ হবে** (rule 24 — generic non-Git vocabulary, VCS-নির্দিষ্ট না): importer/exporter → "ইম্পোর্ট করা টুল"/"এক্সপোর্ট করা টুল" বা প্রসঙ্গভেদে বর্ণনামূলক বাংলা, parser → "পার্সার" (প্রচলিত loanword) বা "বিশ্লেষক" প্রসঙ্গভেদে, bridge (git svn-এর metaphor) → ব্রিজ (প্রচলিত loanword), workspace (generic sense, Perforce-specific client spec workspace না) → "workspace" প্রসঙ্গে বিচার্য। Script → English (Release entry-র "build/release script" precedent অনুযায়ী)।
- **2026-08-08 (numbers rule reversed)** — সম্পূর্ণ বই-জোড়া multi-agent review-এ ধরা পড়েছিল যে ~17 ফাইলে প্রোজে সংখ্যা (২০২০, ১০০, ৪০ ইত্যাদি) STYLE_GUIDE rule 16 ("English digits") ভেঙেছে, বাকি বইয়ে যেখানে English digit প্রতিষ্ঠিত। ফিক্স প্রস্তুত করে দেখানোর পর user সিদ্ধান্ত নিলেন উল্টো দিকে যাওয়ার — prose-এ Bangla digit (১০, ২০২০)-ই থাকবে, এটা বাংলা টেকনিক্যাল লেখায় বেশি স্বাভাবিক পড়ে। rule 16 সেই অনুযায়ী rewrite করা হয়েছে। এই সিদ্ধান্ত prose-নির্দিষ্ট — code block, command output, version number, আর upstream থেকে literal কপি করা কোনো টেক্সটের ভেতরের digit অপরিবর্তিত (English) থাকবে, কারণ ওগুলো byte-for-byte upstream-এর সাথে মিলতে হবে।
- **2026-08-08 (পুরো বই full multi-agent review + ফিক্স)** — Chapter 1-10, Appendix A/B/C, আর front matter — সব ফাইল upstream-এর বিপরীতে লাইন-বাই-লাইন cross-check করা হয়েছে (11+ background agent দিয়ে, প্রতিটা chapter আলাদাভাবে)। কোনো mistranslation বা direction-reversal bug পাওয়া যায়নি। যা ফিক্স করা হয়েছে: (1) `history` শব্দ prose-এ সর্বত্র "ইতিহাস"-এ transliterate করা হয়েছে (আগে বেশিরভাগ জায়গায় English "history" ছিল, GLOSSARY-র পুরনো সিদ্ধান্তের বিপরীতে) — literal branch/remote নাম (`replace.asc`-এ ব্যবহৃত "history" নামের ব্রাঞ্চ) আর Go সোর্স কোড (`go-git.asc`) বাদে; (2) `objects.asc`-এ "object" ভুলবশত "অবজেক্ট"-এ transliterate হয়ে গিয়েছিল ৩০+ জায়গায়, chapter 10 prep-এর সিদ্ধান্ত অনুযায়ী ফিরিয়ে English করা হয়েছে; (3) `bundling.asc`/`credentials.asc`/`debugging.asc`-এর একদম শেষে একটা stray literal `</content>` ট্যাগ (কোনো আগের টুল-প্রসেসিং থেকে leak হওয়া আর্টিফ্যাক্ট) মুছে ফেলা হয়েছে; (4) rule 25 (প্রতিটা→প্রতিটি) আর rule 24 (powerful/useful/effective/বিহেভিয়ার-এর মতো generic adjective native বাংলায়) ch9/ch10/Appendix A-তে যেসব নির্দিষ্ট instance flag হয়েছিল সেগুলো ফিক্স করা হয়েছে (পুরো sweep না, শুধু flagged instance); (5) আরও প্রায় ২০টা ছোট inconsistency — untranslated heading (Tagging, Short Status, Subversion Branching), heading collision (দুটো আলাদা সেকশনে "ফাইল সরানো"), typo (referenc), "শেল"→shell, "রিপ্লে"→replay, "pull down"→"পুল করে আনা", ইত্যাদি — সবই ফিক্স করা হয়েছে। প্রতিটা ফিক্সের পর code block content upstream-এর সাথে byte-for-byte মিলিয়ে verify করা হয়েছে।
